...
def test_day_reply_keeps_repeated_meals_as_separate_rows() -> None:
    meals = [
        DayMeal(
            "спагеті",
            420,
...
def test_reply_warns_when_kcal_and_macros_differ_over_configured_threshold() -> None:
    meal = calculate_meal(
        FoodAnalysis(
            is_food=True,
            meal_name="продукт",
    ...
    warned = format_reply(meal, mismatch_threshold_percent=10)
    hidden = format_reply(meal, mismatch_threshold_percent=80)
...
def test_monthly_contributions_use_daily_average_for_weight_and_calories() -> None:
    ...
    assert "<li>Фрукти, 26600 г (887 г/день)  🔥 К 308 кк/день</li>" in reply
    assert "<li>Фрукти, 26600 г (887 г/день)  🥩 Б 6 г/день</li>" in reply
