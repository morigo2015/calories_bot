...
- валідна їжа отримує три компактні рядки, одиниці `кк`, кнопку `Видалити` і
  створює один рядок;
- точні калорії для сиру — 60, йогурту — 130, батончика — 171;
- лише оцінена вага має `≈`, а калорійність на 100 г показується як `#N`;
- помилки формату й нехарчовий текст не створюють рядків.

## Доба, групування та видалення

1. Перевірити, що після `timestamp` є нативна date-колонка `day` у форматі
   `yyyy-mm-dd`; повідомлення до 01:00 належить попередній даті.
2. Надіслати три повідомлення з однаковою назвою страви та виконати `/day`.
   Очікувати один агрегований рядок із сумою і `×3`.
3. Натиснути `Видалити` під одним із прийомів. Рядок має зникнути з Sheets,
   вихідне повідомлення — з Telegram, а відповідь змінитися на `Видалено` та
   актуальне `=== N кк`.
4. Повторний callback не має спричинити помилку або змінити підсумок.
5. Відредагувати вже оброблене повідомлення: нового рядка й відповіді бути не
   повинно.
