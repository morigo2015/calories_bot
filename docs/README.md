...
Актуальний опис продукту: `docs/PRODUCT.md`. Детальна технічна специфікація:
`docs/TZ.md`.
...
`#120`, `120#` і природна фраза `120 ккал на 100 грам` означають
120 ккал/100 г. Якщо ваги чи калорійності немає, бот оцінює її та позначає
такі числа знаком `≈`. У відповіді наводиться розрахунок для кожного компонента.
Числа, записані словами українською або російською, нормалізуються до цифр перед
аналізом.
Також підтримується одне фото з caption або без нього. Фотоальбоми
й зображення як document ігноруються.

Користувацькі команди:

- `/start <invite_token>` — активувати invite; повторний `/start` показує коротке привітання;
- `/help` — основний сценарій і приклади;
- `/tips` — додатковий синтаксис і можливості;
- `/day` — згрупований підсумок поточної персональної облікової доби.
- `/week` — підсумок поточного облікового дня і шести попередніх;
...
Відома попередня схема без `daily_kcal_goal` мігрується автоматично; для
наявних користувачів ціль спочатку порожня.
