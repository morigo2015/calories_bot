...
`NUTRITION_MISMATCH_THRESHOLD_PERCENT`; вхідні значення не виправляються.
...
`/day` будується безпосередньо з рядків журналу. Кожен із чотирьох показників
має `<details>` зі списком окремих прийомів. Для калорій і білка доступні
цілі та progress bars. Navigation callback показує сусідні дати, але не
майбутнє.
