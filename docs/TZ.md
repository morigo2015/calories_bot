...
**Статус:** актуальна реалізація станом на 23 вересня 2026 року.
...
5. уточнення основи КБЖВ (`per_100g` або `portion`);
6. дата чи значення витрати;
7. поле налаштувань;
8. звичайний опис їжі.
...
Нові рядки зберігають повний `MealResult` у `items_json`: компоненти, загальні
КБЖВ, origins, basis та авторитетні значення всієї порції. Читання лишається
сумісним зі старим форматом, де в цій клітинці був лише JSON-масив
компонентів. Плоскі колонки калорій лишаються для сумісності. Legacy-схема без
`day` мігрується вставленням колонки й backfill.
...
Один аналіз зберігається одним рядком із ID Telegram-повідомлення, навіть якщо
LLM повернув кілька компонентів. Сумісний `normalized_request` містить
`simple_meal:v1` із позицією `0` і кількістю `1`. Повторна доставка тієї самої
події відновлює картку без дублювання. Старі багаторядкові записи з
компонентними від’ємними ID продовжують читатися й відтворюватися без
міграції історії.

## 6. Аналіз і розрахунок

`normalize_input()` виділяє явні вагу, КБЖВ, basis та числа словами. Підтримує
цілі й дробові значення з крапкою або комою; вага може бути в будь-якій
частині опису. Скорочений запис К/Б/Ж/В без basis лишається `per_100g` для
зворотної сумісності. Природні КБЖВ без однозначної основи спричиняють
`NutritionBasisRequired`; handler зберігає початковий payload і продовжує той
самий запит після callback `nutrition-basis:per_100g|portion`.

Analyzer отримує нормалізований текст, source IDs явних значень і, за потреби,
bytes найбільшого фото. `FoodAnalysis.portion_nutrition` містить лише явно
задані авторитетні КБЖВ усієї з’їденої порції; модель не має права вигадувати
ці поля. Source IDs перевіряються локально та використовуються рівно один раз.

LLM повертає один `FoodAnalysis`, що може містити до 20 компонентів;
`calculate_meal()` локально й детерміновано обчислює їхню суму. Явні поля
`portion_nutrition` перекривають відповідні розраховані totals, а неявні поля
і далі беруться із компонентів. Origin `user_text` має пріоритет над `image`,
...
`SavedMeal.base_meal` зберігає весь `MealResult` і може містити один або кілька
компонентів. `list_meals()` сортує спочатку закріплені, потім звичайні;
усередині групи більший `sort_order` іде вище. Новий шаблон отримує найбільший
порядок.
...
`scale_meal()` пропорційно перераховує готовий одно- або багатокомпонентний
результат і явні КБЖВ порції без LLM.
...
`/day` будується безпосередньо з рядків журналу. Кожен із чотирьох показників має `<details>` зі
списком окремих прийомів. У верхньому `<summary>` одиниці виміру не показуються; для калорій і білка цілі та
...
Garmin cache заново завантажує сім завершених днів на початку бот-доби.
Hourly job додатково перевіряє останній завершений день, щоб підхопити
пізню синхронізацію пристрою, і надолужує пропущене оновлення після рестарту.
Garmin використовується як fallback лише для адміністратора, а персональний
worksheet `/burn` має пріоритет.
