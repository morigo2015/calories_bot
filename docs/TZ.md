...
Для потенційно тривалих операцій після перевірки доступу бот підтримує
стандартний індикатор `typing…`, не додаючи реакцій або окремих технічних
повідомлень у чат. Загальна тривалість операції записується в application log.
...
частині опису. Повний набір К/Б/Ж/В (або У) без ваги та basis означає КБЖВ усієї порції; інший
скорочений запис без basis лишається `per_100g`. Природні КБЖВ без однозначної основи спричиняють
