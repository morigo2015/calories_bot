...
Перший рядок `/day` має заголовок `За сьогодні (ДД.ММ)` або
`За день (ДД.ММ)` для історичної дати й містить суму, ціль і залишок.
Inline-кнопки перемикають сусідні календарні дати; на сьогодні кнопки
переходу вперед немає.
...
одного компонента. Після успішного вибору зі saved/recent повідомлення зі
списком видаляється.
...
ID надісланих денних зведень зберігаються локально. Після надсилання нового
зведення бот видаляє всі раніше збережені повідомлення з інтервалу поточної
персональної облікової доби (`day_start` включно, наступний `day_start` не
включно), а потім запам’ятовує ID нового зведення.
