...
Для потенційно тривалих операцій після перевірки доступу бот ставить реакцію
`👀`, підтримує `typing…` і через 1,2 секунди показує один беззвучний
контекстний статус, якщо відповідь ще не готова. Після надсилання результату
тимчасовий статус видаляється. Помилки цього UX-feedback не переривають
основну операцію; її загальна тривалість і факт показу статусу записуються в
application log.
...
Реєстр завантажується в пам’ять під час запуску. Звичайні перевірки доступу не
читають Google Sheets повторно; після створення invite, активації, зміни
налаштувань, block/unblock або видалення бот перечитує реєстр і оновлює кеш.
Після ручної зміни worksheet бот потрібно перезапустити.
